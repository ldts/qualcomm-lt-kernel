				    ...
				    enum ieee80211_roc_type type)
{
	...
	if (vif->p2p) {
		...
		dwell_time = duration;
		/* Include the room for scan complete delay in scheduling scan
		 * timeout worker. Otherwise, scan stop will be sent even
		 * before fw is done with the off-channel scanning.
		 */
