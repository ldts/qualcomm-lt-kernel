...
u8 ath10k_wmi_tlv_tpc_final_get_rate(struct ath10k *ar,
				     struct wmi_tlv_pdev_tpc_final_event *ev,
				     u32 rate_idx, u32 num_chains,
				     u32 rate_code, u8 type, u32 pream_idx)
{
	u8 tpc, num_streams, preamble, ch, stm_idx, pream;
	static const char pream_str[][6] = { "CCK",
					     ...
					     "VHT80",
					     "HTCUP" };
	struct wmi_tlv_pdev_tpc_final_ctl *ctl_tbl;

	ctl_tbl = (struct wmi_tlv_pdev_tpc_final_ctl *)((u8 *)ev
					+ sizeof(struct wmi_tlv)
					+ ev->tlv_fixed_param.len);
	...
	if (__le32_to_cpu(ev->chan_freq) <= 2483) {
		if (!(strcmp(pream_str[pream_idx], "CCK")))
			pream = 0;
		if (!(strcmp(pream_str[pream_idx], "OFDM")))
			pream = 1;
		if (!(strcmp(pream_str[pream_idx], "HT20")) ||
		    !(strcmp(pream_str[pream_idx], "VHT20")))
			pream = 2;
		if (!(strcmp(pream_str[pream_idx], "HT40")) ||
		    !(strcmp(pream_str[pream_idx], "VHT40")))
			pream = 3;
		if (!(strcmp(pream_str[pream_idx], "HT80")) ||
		    !(strcmp(pream_str[pream_idx], "VHT80")))
			pream = 4;
	}
	...
	if (__le32_to_cpu(ev->chan_freq) >= 5180) {
		...
		if (!(strcmp(pream_str[pream_idx], "HT80")) ||
		    !(strcmp(pream_str[pream_idx], "VHT80")))
			pream = 3;
		if (!(strcmp(pream_str[pream_idx], "HTCUP")))
			pream = 4;
	}
	...
	if (pream == 4)
		...
		tpc = min_t(u8,
			    min_t(u8, ev->rates_array[rate_idx],
				  ev->max_reg_allow_pow[ch]),
...
static void
ath10k_wmi_tlv_tpc_final_disp_tbl(struct ath10k *ar,
				  struct wmi_tlv_pdev_tpc_final_event *ev,
...
void
ath10k_wmi_tlv_event_tpc_final_table(struct ath10k *ar, struct sk_buff *skb)
{
	u16 pream_table[WMI_TPC_PREAM_TABLE_MAX];
	struct wmi_tlv_pdev_tpc_final_event *ev;
	u8 rate_code[WMI_TPC_RATE_MAX_TLV];
	struct ath10k_tpc_stats *tpc_stats;
	u32 num_tx_chain;

	ev = (struct wmi_tlv_pdev_tpc_final_event *)skb->data;
...
void
ath10k_wmi_tlv_event_tpc_config_dispatch(struct ath10k *ar, struct sk_buff *skb)
{
	struct wmi_tlv_pdev_tpc_final_event *ev;
	u32 flags, tag;
	...
	ev = (struct wmi_tlv_pdev_tpc_final_event *)skb->data;
	flags = __le32_to_cpu(ev->flags);
	tag = __le16_to_cpu(ev->tlv_fixed_param.tag);
	if (tag == WMI_TLV_TAG_STRUCT_PDEV_TPC_CONFIG_EVENT &&
	    ((flags & WMI_TPC_CONFIG_EVENT_FLAG_IF_MASK) ==
	    WMI_TPC_CONFIG_EVENT_FLAG_IF_V1)) {
		ath10k_wmi_tlv_event_tpc_final_table(ar, skb);
	} else {
...
static void ath10k_wmi_tlv_op_rx(struct ath10k *ar, struct sk_buff *skb)
{
	...
	case WMI_TLV_PDEV_TPC_CONFIG_EVENTID:
		ath10k_wmi_tlv_event_tpc_config_dispatch(ar, skb);
		break;
...
static struct sk_buff *
ath10k_wmi_tlv_op_gen_tpc_final_table(struct ath10k *ar, u32 param)
{
	struct wmi_tlv_pdev_get_tpc_config_cmd *cmd;
	struct wmi_tlv *tlv;
	struct sk_buff *skb;
	...
	tlv = (void *)skb->data;
	tlv->tag = __cpu_to_le16(WMI_TLV_TAG_STRUCT_PDEV_GET_TPC_CONFIG_CMD);
	tlv->len = __cpu_to_le16(sizeof(*cmd));
	cmd = (void *)tlv->value;
	cmd->pdev_id = 0;
	cmd->param = __cpu_to_le32(WMI_TPC_CONFIG_PARAM_MULTI_AS_FINAL);
...
static struct wmi_pdev_param_map wmi_tlv_pdev_param_map = {
	...
	.aggr_burst = WMI_TLV_PDEV_PARAM_AGGR_BURST,
