...
#define PHY_TX_DELAY		0x05
#define	PHY_RX_DELAY		0x00

#define PHY_TX_DELAY_WR_MASK	0xfeff
#define PHY_TX_DELAY_MASK	0x1
#define PHY_RX_DELAY_WR_MASK	0x7fff
#define PHY_RX_DELAY_MASK	0x1

static void ethqos_set_phy_delay(struct mii_bus *mii, unsigned int addr)
{
	unsigned int data, tx_delay = 0, rx_delay = 0;

	/* set phy registers with delay passed */
	mii->write(mii, addr, PHY_DEBUG_PORT_ADDR, PHY_TX_DELAY);

	data = mii->read(mii, addr, PHY_DEBUG_PORT_DATA);
	data = (data & PHY_TX_DELAY_WR_MASK) |
		((tx_delay & PHY_TX_DELAY_MASK) << 8);
	mii->write(mii, addr, PHY_DEBUG_PORT_DATA, data);

	mii->write(mii, addr, PHY_DEBUG_PORT_ADDR, PHY_RX_DELAY);

	data = mii->read(mii, addr, PHY_DEBUG_PORT_DATA);
	data = (data & PHY_RX_DELAY_WR_MASK) |
		((rx_delay & PHY_TX_DELAY_MASK) << 15);
	mii->write(mii, addr, PHY_DEBUG_PORT_DATA, data);
}

static const struct clk_bulk_data ethqos_clks[] = {
	{ .id = "rgmii" },
	{ .id = "stmmaceth" },
	{ .id = "pclk" },
	{ .id = "ptp_ref" },
...
static int qcom_ethqos_probe(struct platform_device *pdev)
{
	...
	ethqos->pdev = pdev;
	ethqos->clks = devm_kmemdup(&pdev->dev, ethqos_clks,
				    sizeof(ethqos_clks), GFP_KERNEL);
	if (!ethqos->clks) {
		ret = -ENOMEM;
		goto err_mem;
	}
	ethqos->num_clks = ARRAY_SIZE(ethqos_clks);
	...
	ret = clk_bulk_prepare_enable(ethqos->num_clks, ethqos->clks);
	if (ret)
		goto err_mem;
	...
	plat_dat->set_phy_quirk = ethqos_set_phy_delay;
...
err_clk:
	clk_bulk_disable_unprepare(ethqos->num_clks, ethqos->clks);
