...
static int fastrpc_req_mmap(struct fastrpc_user *fl, char __user *argp)
{
	...
	if (req.flags != ADSP_MMAP_ADD_PAGES) {
		dev_err(NULL, "flag not supported 0x%x\n", req.flags);
		return -EINVAL;
	}

	if (req.vaddrin) {
		dev_err(NULL, "adding user allocated pages is not supported\n");
		return -EINVAL;
	}

	err = fastrpc_buf_alloc(fl, fl->sctx->dev, req.size, &buf);
	if (err) {
		dev_err(NULL, "failed to allocate buffer\n");
		return err;
	}
...
static int fastrpc_req_munmap(struct fastrpc_user *fl, char __user *argp)
{
	...
	if (!buf) {
		dev_err(fl->sctx->dev, "mmap not in list\n");
		return -EINVAL;
	}
