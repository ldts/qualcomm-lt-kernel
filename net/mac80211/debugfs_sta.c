...
void ieee80211_rx_h_sta_stats(struct sta_info *sta, struct sk_buff *skb)
{
	...
	if (status->bw & (RATE_INFO_BW_5 | RATE_INFO_BW_10))
		goto out;
	...
	if (status->encoding & RX_ENC_VHT) {
		...
		if (status->nss > IEEE80211_VHT_NSS_NUM ||
		    !status->nss)
			goto out;

		sta->rx_nss_pkt[status->nss - 1]++;
		sta->rx_nss_byte[status->nss - 1] += pkt_len;

		sta->rx_mcs_pkt[status->rate_idx]++;
		sta->rx_mcs_byte[status->rate_idx] += pkt_len;

		i = status->rate_idx * 8 + 8 * 10 * (status->nss - 1);
		i += bw_idx * 2 + gi_idx;
		sta->rx_rate_pkt[i]++;
